...
async def create_working_graphic_endpoint(
        working_graphic: WorkingGraphicCreate,
        db: AsyncSession = Depends(get_db),
        user: User = Depends(current_user)
):
    if not user:
...
async def get_working_graphics_endpoint(
        skip: int = 0,
        limit: int = 10,
        db: AsyncSession = Depends(get_db),
        user: User = Depends(current_user)
...
async def get_working_graphic_endpoint(
        working_graphic_id: int,
        db: AsyncSession = Depends(get_db),
        user: User = Depends(current_user)
):
...
async def update_working_graphic_endpoint(
        working_graphic_id: int,
        working_graphic: WorkingGraphicUpdate,
        db: AsyncSession = Depends(get_db),
        user: User = Depends(current_user)
):
    if not user:
...
async def delete_working_graphic_endpoint(
        working_graphic_id: int,
        db: AsyncSession = Depends(get_db),
        user: User = Depends(current_user)
):
